...
Deprecated, think this was from testing some preliminary analysis
```{}
#ape::comparePhylo(gene_tree_subset, hq_tree_subset, plot = TRUE)
...
( umap_plot_data %>% 
    ...
    geom_point() +
    theme_bw()+
    labs(title = "UMAP of Microcystis shared gene content",
...
(primer_prod_tree <- ggtree(iqtree,layout = "circular") %<+% mcye_for_plot + 
  geom_rootedge() +
...
ggsave("results/iqtree_mcyE.pdf",width = 2, height = 2, scale = 10)
...
alignments <- c(hist_gene_alignments, full_gene_alignments, amplicon_alignments, cai23_alignments,cai23_amplicon_alignments)
...
hist_dists <- map_df(tree_files, compare_to_hq_combined) %>% 
  ...
  mutate(clust_info_dist = min(clust_info_dist),
         ...
         percent_of_genomes = round(n_shared_genomes / 181 * 100))
