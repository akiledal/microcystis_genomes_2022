...
data_for_plot <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1Mj3IR_CAMZsZbYq5Z2KY4znf6N6S192hTq3JJOa0UeU/edit#gid=1474076659", sheet = "for_paper_figure") %>% 
  mutate(across(is.numeric, ~replace_na(.x, 0))) # Replace any missing values with absent
...
full_data_for_plot_long <- data_for_plot %>% 
  ...
  group_by(gene) %>% 
  mutate(percent_present = sum(pres_abs) / n()) %>% 
...
(strain_gene_plot <- simp_plot_data %>%
    mutate(pres_abs = if_else(pres_abs ==1, "Present", "Absent"),
           gene_group = if_else(is.na(gene_group),"other", gene_group)) %>% 
  ggplot(aes(gene, factor(pubID,levels = tip_order$label, ordered = TRUE), fill = pres_abs, color = factor(gene_group, levels = c(gene_group_order,"other"), ordered = TRUE))) +
  geom_tile(size = 1) +
...
(strain_gene_plot_no_colors <- simp_plot_data %>%
    mutate(pres_abs = if_else(pres_abs ==1, "Present", "Absent"),
           gene_group = if_else(is.na(gene_group),"other", gene_group)) %>% 
  ggplot(aes(gene, factor(pubID,levels = tip_order$label, ordered = TRUE), fill = pres_abs, ordered = TRUE)) +
...
(full_strain_gene_plot <- full_simp_plot_data %>%
    mutate(pres_abs = if_else(pres_abs ==1, "Present", "Absent"),
           gene_group = if_else(is.na(gene_group),"other", gene_group)) %>% 
  ggplot(aes(gene, factor(pubID,levels = tip_order$label, ordered = TRUE), fill = pres_abs, color = factor(gene_group, levels = c(full_gene_group_order,"other"), ordered = TRUE))) +
  geom_tile(size = 1) +
  #geom_tile(color = "black") + # black lines around cells
  scale_fill_manual(values = c("white", "black")) + 
...
(hets_plot <- het_plot_data %>% 
  ggplot(aes(factor(genus, rev(c("",het_order)), ordered = TRUE) %>% forcats::fct_relevel("Microcystis"), factor(pubID, levels = tip_order$label, ordered = TRUE) , fill = rel_abund)) +
  geom_tile() +
  geom_text(data = mlr_strings %>% select(genus, culture = "pubID", mlr), aes(factor(genus, het_order, ordered = TRUE), factor(culture, levels = tip_order$label, ordered = TRUE), label = mlr), color = "white", inherit.aes = FALSE, position = position_nudge(x = 0, y = -0.2)) +
    geom_text(data = mlr_strings %>% select(genus, culture = "pubID", katG), aes(factor(genus, rev(het_order), ordered = TRUE), factor(culture, levels = tip_order$label, ordered = TRUE), label = katG), color = "white", inherit.aes = FALSE, position = position_nudge(x = 0, y = 0.2)) +
...
(hets_plot <- het_plot_data %>% 
  ggplot(aes(factor(culture, levels = tip_order$label, ordered = TRUE), rel_abund, fill = factor(genus, rev(het_order), ordered = TRUE) %>% forcats::fct_relevel("Microcystis",after = Inf))) +
